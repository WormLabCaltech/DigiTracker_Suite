...
#define RECFPS 6                     // initial record rate (CJC 2008-02-20 was 15)
...
// WAIT is number of image grab/process cycles to skip before trying to track (look for image differences) again.
int WAIT[4] = {10,10,10,10};		  // CJC 5-07-2008 reduced number of wait cycles.
	...
	: CDialog(CExampleDlg::IDD, pParent)
{
	...
	m_restart = 1;			// Initialize the tracking system by forcing a restart right away.  (was 90;)
	m_restartTime = GetTickCount() + 10000;	// Set start of reset to 10 seconds from now (to allow stage to start)

	m_settlingTime = 750;	// Time (in milliseconds) to pause before attempting to track again.
	m_track = 0;
	m_image = 1;
...
///////////////////////////////////////////////////////////////////////////
   ...
   m_Slider.SetPos(2);			//CJC 2008-02-20 was 3
...
// CJC 10-12-04
   ...
   // Alert user if DigiTracker is using non-standard fps or shutoff time values
   if ( (m_autoShutOffSpinner.GetPos() != RECTIME) || (m_spinner.GetPos() != RECFPS) )
	   AfxMessageBox("ALERT: Check values for\n"
		   "\n"
		   ...
		   "\n"
		   "\n"
		   "==> May be set to NON-STANDARD Values\t");
		...
		{			
			...
			if( CurrentSuccess )
			{
				...
				if( m_restart > 0 )
				{
					...
					if ( (long)(GetTickCount() - m_restartTime) > m_settlingTime)
					{
						// Clear restart flag
						m_restart = 0;
					}
...
//				else if ((m_nProcessingCount % m_updateDelay) == 0)
				...
				if( m_track )
					if( (x != -1) && (y != -1) )
						...
						{
						   // Set restart flag and note time
						   m_restart = 1;
						   m_restartTime = GetTickCount();
...
// CJC 11-15-07 trying
				// CJC 11-15-07 discovered that PostMessage makes "Finished
				// Recording" show up (in Debug version), but in Release version, 
				// on SECOND time using auto-shutoff feature, program crashes.  So,
				// placing PostMessage call only for Debug!
#ifdef _DEBUG
...
//#ifdef _DEBUG	// CJC 10-27-08
//			AfxMessageBox("Before checking equality");	//CJC
//			bool equal;
//			equal = (PreviousCameraSN == CurrentCameraSN);		//CJC
//			AfxMessageBox("Before checking equality");	//CJC
//#endif
//			if (equal)
			...
			else	// different camera
			{
			CHECKSTATETYPE BogusAutoExposureState;	// temporary (i.e. unused) CHECKSTATETYPE

				// Read archive into a BOGUS AutoExposureState structure to allow 
				// archive to output all its data (and prevent a program crash when it can't).
				ar >> BogusAutoExposureState.brightnessCheck; 
				ar >> BogusAutoExposureState.gainCheck;
				ar >> BogusAutoExposureState.shutterCheck;
				ar >> BogusAutoExposureState.autoExposureCheck;
				ar >> BogusAutoExposureState.gammaCheck;
				ar >> BogusAutoExposureState.sharpnessCheck;
			}
		...
		{
			CString msg;
            msg.Format(_T("CArchiveException: %s"), e.m_cause);
#ifdef _DEBUG	// CJC 10-27-08
            AfxMessageBox(msg);
#endif
} 
		...
		{
			CString msg;
            msg.Format(_T("CFileException: %d"), e.m_lOsError);
#ifdef _DEBUG	// CJC 10-27-08
            AfxMessageBox(msg);
#endif
}
...
//		locationDlg->DoModal();
		...
		AfxMessageBox(
			...
			"After the stage shifts, hold the 'Ctrl' key and left-click on the same feature a second time.\n"
			"\n"
...
//		AfxMessageBox("0");		// CJC 2008-02-15
