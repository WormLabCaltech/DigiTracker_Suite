...
typedef REALTRI **triangle;            /* Really:  typedef triangle *triangle   */
...
typedef REALTRI **shelle;                  /* Really:  typedef shelle *shelle   */
...
struct badface {
  ...
  REALTRI key;                             /* cos^2 of smallest (apical) angle. */
...
struct event {
  REALTRI xkey, ykey;                              /* Coordinates of the event. */
...
REALTRI xmin, xmax, ymin, ymax;                              /* x and y bounds. */
REALTRI xminextreme;        /* Nonexistent x value used as a flag in sweepline. */
...
REALTRI splitter;       /* Used to split REAL factors for exact multiplication. */
REALTRI epsilon;                             /* Floating-point machine epsilon. */
REALTRI resulterrbound;
REALTRI ccwerrboundA, ccwerrboundB, ccwerrboundC;
REALTRI iccerrboundA, iccerrboundB, iccerrboundC;
...
#define elemattribute(triedge, attnum)                                        \
  ((REALTRI *) (triedge).tri)[elemattribindex + (attnum)]

#define setelemattribute(triedge, attnum, value)                              \
  ((REALTRI *) (triedge).tri)[elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(triedge)  ((REALTRI *) (triedge).tri)[areaboundindex]

#define setareabound(triedge, value)                                          \
  ((REALTRI *) (triedge).tri)[areaboundindex] = value
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'q') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            minangle = (REALTRI) strtod(workstring, (char **) NULL);
	  } else {
            minangle = 20.0;
	  }
	}
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            maxarea = (REALTRI) strtod(workstring, (char **) NULL);
            if (maxarea <= 0.0) {
			  ...
			  int alignment)
{
  ...
  wordsize = (pool->itemwordtype == POINTER) ? sizeof(VOID *) : sizeof(REALTRI);
...
VOID *poolalloc(struct memorypool *pool)
{
  ...
  if (pool->deaditemstack != (VOID *) NULL) {
    ...
    if (pool->itemwordtype == POINTER) {
      ...
      pool->nextitem = (VOID *) ((REALTRI *) pool->nextitem + pool->itemwords);
    }
...
VOID *traverse(struct memorypool *pool)
{
  ...
  if (pool->itemwordtype == POINTER) {
    ...
    pool->pathitem = (VOID *) ((REALTRI *) pool->pathitem + pool->itemwords);
  }
  pool->pathitemsleft--;
  return newitem;
}
...
void initializepointpool()
{
  ...
  pointmarkindex = ((mesh_dim + nextras) * sizeof(REALTRI) + sizeof(int) - 1)
                 / sizeof(int);
  ...
  poolinit(&points, pointsize, POINTPERBLOCK,
           (sizeof(REALTRI) >= sizeof(triangle)) ? FLOATINGPOINT : POINTER, 0);
}
...
void initializetrisegpools()
{
  ...
  elemattribindex = (trisize + sizeof(REALTRI) - 1) / sizeof(REALTRI);
  ...
  if (vararea) {
    trisize = (areaboundindex + 1) * sizeof(REALTRI);
  } else if (eextras + regionattrib > 0) {
    trisize = areaboundindex * sizeof(REALTRI);
  }
...
{
  REALTRI Q;
  INEXACT REALTRI Qnew;
  INEXACT REALTRI hh;
  INEXACT REALTRI bvirt;
  REALTRI avirt, bround, around;
...
int scale_expansion_zeroelim(int elen,
							 ...
							 REALTRI *h)
{
  INEXACT REALTRI Q, sum;
  REALTRI hh;
  INEXACT REALTRI product1;
  REALTRI product0;
  int eindex, hindex;
  REALTRI enow;
  INEXACT REALTRI bvirt;
  REALTRI avirt, bround, around;
  INEXACT REALTRI c;
  INEXACT REALTRI abig;
  REALTRI ahi, alo, bhi, blo;
  REALTRI err1, err2, err3;
...
REALTRI estimate(int elen, REALTRI *e)
{
  REALTRI Q;
  int eindex;
...
REALTRI counterclockwiseadapt(point pa, point pb, point pc, REALTRI detsum)
{
  INEXACT REALTRI acx, acy, bcx, bcy;
  REALTRI acxtail, acytail, bcxtail, bcytail;
  INEXACT REALTRI detleft, detright;
  REALTRI detlefttail, detrighttail;
  REALTRI det, errbound;
  REALTRI B[4], C1[8], C2[12], D[16];
  INEXACT REALTRI B3;
  int C1length, C2length, Dlength;
  REALTRI u[4];
  INEXACT REALTRI u3;
  INEXACT REALTRI s1, t1;
  REALTRI s0, t0;
  ...
  REALTRI ahi, alo, bhi, blo;
  REALTRI err1, err2, err3;
  INEXACT REALTRI _i, _j;
  REALTRI _0;

  acx = (REALTRI) (pa[0] - pc[0]);
  bcx = (REALTRI) (pb[0] - pc[0]);
  acy = (REALTRI) (pa[1] - pc[1]);
  bcy = (REALTRI) (pb[1] - pc[1]);
...
REALTRI counterclockwise(point pa, point pb, point pc)
{
  REALTRI detleft, detright, det;
  REALTRI detsum, errbound;
...
REALTRI incircleadapt(point pa, point pb, point pc, point pd, REALTRI permanent)
{
  INEXACT REALTRI adx, bdx, cdx, ady, bdy, cdy;
  REALTRI det, errbound;

  INEXACT REALTRI bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  REALTRI bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  REALTRI bc[4], ca[4], ab[4];
  INEXACT REALTRI bc3, ca3, ab3;
  REALTRI axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  REALTRI bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  REALTRI cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  REALTRI abdet[64];
  int ablen;
  REALTRI fin1[1152], fin2[1152];
  REALTRI *finnow, *finother, *finswap;
  int finlength;

  REALTRI adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
  INEXACT REALTRI adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  REALTRI adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  REALTRI aa[4], bb[4], cc[4];
  INEXACT REALTRI aa3, bb3, cc3;
  INEXACT REALTRI ti1, tj1;
  REALTRI ti0, tj0;
  REALTRI u[4], v[4];
  INEXACT REALTRI u3, v3;
  REALTRI temp8[8], temp16a[16], temp16b[16], temp16c[16];
  REALTRI temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  REALTRI axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  REALTRI bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  REALTRI cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  REALTRI axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  REALTRI axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  REALTRI axtbctt[8], aytbctt[8], bxtcatt[8];
  REALTRI bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  REALTRI abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  REALTRI abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
  INEXACT REALTRI abtt3, bctt3, catt3;
  REALTRI negate;
  ...
  adx = (REALTRI) (pa[0] - pd[0]);
  bdx = (REALTRI) (pb[0] - pd[0]);
  cdx = (REALTRI) (pc[0] - pd[0]);
  ady = (REALTRI) (pa[1] - pd[1]);
  bdy = (REALTRI) (pb[1] - pd[1]);
  cdy = (REALTRI) (pc[1] - pd[1]);
...
REALTRI incircle(point pa, point pb, point pc, point pd)
{
  REALTRI adx, bdx, cdx, ady, bdy, cdy;
  REALTRI bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  REALTRI alift, blift, clift;
  REALTRI det;
  REALTRI permanent, errbound;
...
void testtriangle(struct triedge *testtri)
{
  ...
  point anglevertex;
  REALTRI dxod, dyod, dxda, dyda, dxao, dyao;
  REALTRI dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
  REALTRI apexlen, orglen, destlen;
  REALTRI angle;
  REALTRI area;
  shelle sptr;                      /* Temporary variable used by tspivot(). */
...
REALTRI circletop(point pa, point pb, point pc, REALTRI ccwabc)
{
  REALTRI xac, yac, xbc, ybc, xab, yab;
  REALTRI aclen2, bclen2, ablen2;
...
void check4deadevent(struct triedge *checktri, struct event **freeevents, 
...
struct splaynode *circletopinsert(struct splaynode *splayroot, struct triedge *newkey, 
								  point pa, point pb, point pc, REALTRI topy)
{
  REALTRI ccwabc;
  REALTRI xac, yac, xbc, ybc;
  REALTRI aclen2, bclen2;
  REALTRI searchpoint[2];
...
int reconstruct(int *trianglelist, REALTRI *triangleattriblist, REALTRI *trianglearealist,
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < eextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        setelemattribute(triangleloop, j,
                         (REALTRI) strtod (stringptr, &stringptr));
      }
#endif /* not TRILIBRARY */
    }
    ...
    if (vararea) {
      ...
      if (*stringptr == '\0') {
        ...
        area = (REALTRI) strtod(stringptr, &stringptr);
      }
#endif /* not TRILIBRARY */
...
void regionplague(REALTRI attribute, REALTRI area)
{
  struct triedge testtri;
  struct triedge neighbor;
  triangle **virusloop;
...
void carveholes(REALTRI *holelist, int holes, REALTRI *regionlist, int regions)
{
  struct triedge searchtri;
  struct triedge triangleloop;
...
void repairencs(int flaws)
{
  ...
  point newpoint;
  enum insertsiteresult success;
  REALTRI segmentlength, nearestpoweroftwo;
  REALTRI split;
...
enum circumcenterresult findcircumcenter(point torg, point tdest, point tapex, point circumcenter,
										 REALTRI *xi, REALTRI *eta)
{
  REALTRI xdo, ydo, xao, yao, xad, yad;
  REALTRI dodist, aodist, addist;
  REALTRI denominator;
  REALTRI dx, dy;
...
void readnodes(char *nodefilename, char *polyfilename, FILE **polyfile)
{
  FILE *infile;
  point pointloop;
  char inputline[INPUTLINESIZE];
  ...
  for (i = 0; i < inpoints; i++) {
    ...
    x = (REALTRI) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("Error:  Point %d has no y coordinate.\n", firstnumber + i);
      exit(1);
    }
    y = (REALTRI) strtod(stringptr, &stringptr);
    pointloop[0] = x;
    pointloop[1] = y;
    ...
    for (j = 2; j < 2 + nextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        pointloop[j] = (REALTRI) strtod(stringptr, &stringptr);
      }
    }
    if (nodemarkers) {
      /* Read a point marker. */
...
void transfernodes(REALTRI *pointlist, REALTRI *pointattriblist,
...
void readholes(FILE *polyfile, char *polyfilename, REALTRI **hlist,
			   int *holes, REALTRI **rlist, int *regions)
{
  REALTRI *holelist;
  REALTRI *regionlist;
  char inputline[INPUTLINESIZE];
  ...
  *holes = (int) strtol (stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (REALTRI *) malloc(2 * *holes * sizeof(REALTRI));
    *hlist = holelist;
    if (holelist == (REALTRI *) NULL) {
      printf("Error:  Out of memory.\n");
      exit(1);
    }
    for (i = 0; i < 2 * *holes; i += 2) {
      ...
      if (*stringptr == '\0') {
        ...
        holelist[i] = (REALTRI) strtod(stringptr, &stringptr);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        holelist[i + 1] = (REALTRI) strtod(stringptr, &stringptr);
      }
    }
  } else {
    *hlist = (REALTRI *) NULL;
  ...
  if ((regionattrib || vararea) && !refine) {
    ...
    *regions = (int) strtol (stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (REALTRI *) malloc(4 * *regions * sizeof(REALTRI));
      *rlist = regionlist;
      if (regionlist == (REALTRI *) NULL) {
        printf("Error:  Out of memory.\n");
        exit(1);
      }
      index = 0;
      for (i = 0; i < *regions; i++) {
        ...
        if (*stringptr == '\0') {
          ...
          regionlist[index++] = (REALTRI) strtod(stringptr, &stringptr);
        }
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
...
void writenodes(REALTRI **pointlist, REALTRI **pointattriblist, int **pointmarkerlist)
...

{
  ...
  if (*pointlist == (REALTRI *) NULL) {
    *pointlist = (REALTRI *) malloc(points.items * 2 * sizeof(REALTRI));
    if (*pointlist == (REALTRI *) NULL) {
      printf("Error:  Out of memory.\n");
      exit(1);
    }
  ...
  if ((nextras > 0) && (*pointattriblist == (REALTRI *) NULL)) {
    *pointattriblist = (REALTRI *) malloc(points.items * nextras * sizeof(REALTRI));
    if (*pointattriblist == (REALTRI *) NULL) {
      printf("Error:  Out of memory.\n");
      exit(1);
    }
  }
...
void writeelements(int **trianglelist, REALTRI **triangleattriblist)
...

{
  ...
  if ((eextras > 0) && (*triangleattriblist == (REALTRI *) NULL)) {
    *triangleattriblist = (REALTRI *) malloc(triangles.items * eextras *
                                          sizeof(REALTRI));
    if (*triangleattriblist == (REALTRI *) NULL) {
      printf("Error:  Out of memory.\n");
      exit(1);
    }
  }
...
void writepoly(char *polyfilename, REALTRI *holelist, int holes, 
			   REALTRI *regionlist, int regions, int argc, char **argv)
...
void writevoronoi(REALTRI **vpointlist, REALTRI **vpointattriblist,
				  int **vpointmarkerlist, int **vedgelist, int **vedgemarkerlist, 
				  REALTRI **vnormlist)
...

{
  ...
  if (*vpointlist == (REALTRI *) NULL) {
    *vpointlist = (REALTRI *) malloc(triangles.items * 2 * sizeof(REALTRI));
    if (*vpointlist == (REALTRI *) NULL) {
      printf("Error:  Out of memory.\n");
      exit(1);
    }
  ...
  if (*vpointattriblist == (REALTRI *) NULL) {
    *vpointattriblist = (REALTRI *) malloc(triangles.items * nextras *
                                        sizeof(REALTRI));
    if (*vpointattriblist == (REALTRI *) NULL) {
      printf("Error:  Out of memory.\n");
      exit(1);
  ...
  *vedgemarkerlist = (int *) NULL;
  ...
  if (*vnormlist == (REALTRI *) NULL) {
    *vnormlist = (REALTRI *) malloc(edges * 2 * sizeof(REALTRI));
    if (*vnormlist == (REALTRI *) NULL) {
      printf("Error:  Out of memory.\n");
      exit(1);
    }
...
void writeoff(char *offfilename, int argc, char **argv)
{
  FILE *outfile;
  struct triedge triangleloop;
  point pointloop;
...
void quality_statistics()
{
  struct triedge triangleloop;
  point p[3];
  REALTRI cossquaretable[8];
  REALTRI ratiotable[16];
  REALTRI dx[3], dy[3];
  REALTRI edgelength[3];
  REALTRI dotproduct;
  REALTRI cossquare;
  REALTRI triarea;
  REALTRI shortest, longest;
  REALTRI trilongest2;
  REALTRI smallestarea, biggestarea;
  REALTRI triminaltitude2;
  REALTRI minaltitude;
  REALTRI triaspect2;
  REALTRI worstaspect;
  REALTRI smallestangle, biggestangle;
  REALTRI radconst, degconst;
  int angletable[18];
  int aspecttable[16];
  int aspectindex;
  int tendegree;
  ...
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
    cossquaretable[i] = cos(radconst * (REALTRI) (i + 1));
...
{
  REALTRI *holearray;                                        /* Array of holes. */
  REALTRI *regionarray;   /* Array of regional attributes and area constraints. */
#ifndef TRILIBRARY
  FILE *polyfile;
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
  ...
  if (poly || convex) {
    ...
    if (nopolywritten || noiterationnum) {
      ...
      if (poly) {
        ...
        out->holelist = (REALTRI *) NULL;
        out->regionlist = (REALTRI *) NULL;
      }
#else /* not TRILIBRARY */
