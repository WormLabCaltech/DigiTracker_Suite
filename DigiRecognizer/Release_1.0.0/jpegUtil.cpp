                 ...
                 CString*   pcsMsg)   //Error msg to return
{
    ...
    *pcsMsg = "";
    ...
    cinfo.image_width      = 320;//lpbi->biWidth;  //Image width and height, in pixels 
    cinfo.image_height     = 240;//lpbi->biHeight;
      ...
      case 1:
         ...
         for (r=0; r < pbBmHdr->biHeight; r++)
         {
            ...
            { 
               ...
               int nBUsed = (p <(nUsed)) ? 8 : nLastBits; 
			   for(b=0; b < nBUsed;b++) 
               { 
                ...
                CString*                    pcsMsg)
{
   ...
   int r=0, p=0, q=0, 
       nUnused=0, nBytesWide=0, nUsed=0,
       nRow=0, nPixel=0;
                    ...
                    CString*   pcsMsg)   //Error msg to return
{
    ...
    *pcsMsg = "";
    ...
    cinfo.input_components = nComponents;              //Color components per pixel
                                             //(RGB_PIXELSIZE - see jmorecfg.h)
    cinfo.in_color_space   = JCS_RGB; 	     //Colorspace of input image
	...
	int r, p, q, nRow, nPixel;

	
	for (r=0; r < height;r++) {
		for (p=0, q=0;p < width; p++, q+=3){ 
			nRow = r*width; 
            nPixel  = nRow + p;

            jsmpArray[r][q] = lpbits[nPixel]; //Red
            jsmpArray[r][q+1] = lpbits[nPixel]; //Green
            jsmpArray[r][q+2] = lpbits[nPixel]; //Blue
        }
    }
