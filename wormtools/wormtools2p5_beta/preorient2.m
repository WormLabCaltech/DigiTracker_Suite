function [x, y, timeV] = preorient2(directory)
...
% 
% 2008-04-04: Version control via Subversion
%   C. J. Cronin 6-03-03
...
for i = 2:nworms
    if fliptest( x(i,:), y(i,:),   x(i-1,:), y(i-1,:))  % Compare current w/previous frame
...
for i = 3:nworms
    if (len(i-1) < 0.2*mcl) & (valid(i) & valid(i-2)) 
        ...
        if fliptest( x(i,:), y(i,:),   x(i-2,:), y(i-2,:))  
