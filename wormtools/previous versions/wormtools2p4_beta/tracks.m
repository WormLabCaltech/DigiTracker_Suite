...
% and never have two points directly above each other
        dwwx=diff(wwx);
        badWormOrient=0;    % Initialize flag.
        if (dwwx(1,1)~=0)   % Check if first two points are directly above each other.
            ndwwx=dwwx/dwwx(1,1);   % If not, normalize against first interval.
            if any(ndwwx<=0)    % Any negatives here indicate a worm in an 
                                % S-shape, violating the monotonically 
                                % increasing or decreasing requirement,
                                % so...
                badWormOrient=1;    % Set flag.
            end
        else
            badWormOrient=1;    % Set flag for first two points sharing an X value.
        end
        
        
        if (badWormOrient>0)    % If a badly oriented worm,...
            ampt(i) = NaN;
        else
