function [vel, theta, mode, velc] = translation3(x, y, mmpp, spf, fpt, timeV)

%TRANSLATION3    Calculate velocity from xy coordinate data.
%   FORMAT: [vel, theta, mode] = translation3(x, y, mmpp, spf, fpt, timeV)
%      where 
...
%      - "timeV" is the vector of time stamps (in seconds) indicating the
%         relative time at which each original video frame was captured.
%         Those images were used to find the body posture and positions
%         described by the rows of coordinates in the x and y matrices, so
%         timeV lists the times at which those postures/positions were
%         recorded. 
%
...
% Establish "abridged" versions of x and y, sampled at every N'th frame:
